...
// How long is an object accessible and rechable?
/**
...
// IMP:WeakMap is fundamentally different in this aspect. It doesn’t prevent garbage-collection of key objects.
/**
 * WeakMap: The first difference between Map and WeakMap is that keys must be objects, not primitive values: IMP
 */
...
// john is removed from memory! IMP

/*
 * Compare it with the regular Map example above. Now if john only exists as the key of WeakMap – 
 ...
 * QUEST: Why such a limitation? That’s for technical reasons.
 * If an object has lost all other references (like john in the code above), then it is to be garbage-collected automatically. 
...
QUEST: Where do we need such a data structure?
Use case: additional data IMP

IMP The main area of application for WeakMap is an additional data storage.
...
For instance, we have code that keeps a visit count for users. 
 ...
 * The downside is that we need to clean cache when the object is not needed any more.
